...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Hz ❤️ Singel Mined" }, status: "idle" });
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Hz Koruma" }).catch();
};
...
client.on("channelCreate", async channel => {
  ...
  channel.delete({reason: "Hz Kanal Koruma"});
